-- Seed: 8 rows into system_prompts
...
VALUES (
  ...
  $BODY$# GRAND MASTER PROMPT: Blue-Ocean YouTube Topic Generator v3.0
...
No markdown. No commentary. Valid JSON only.$BODY$,
  1,
  true,
  'Grand Master Topic Generator v3.0 — full document + JSON output instruction'
)
...
VALUES (
  ...
  $BODY$You are a Senior Domain Expert, YouTube Content Strategist, and Narrative Architect specializing in {niche_category}.
...
- Sentence variety: mix short punchy sentences with longer flowing ones. Monotonous sentence length = monotonous delivery.$BODY$,
  1,
  true,
  'Script Generator v1.0 §3 — system role prompt injected into all 3 passes'
)
ON CONFLICT (prompt_type) DO UPDATE
  SET prompt_text = EXCLUDED.prompt_text,
      version     = system_prompts.version + 1,
...
VALUES (
  'script_pass1',
  $BODY$Write PASS 1 of 3 for a YouTube masterclass titled: {subtopic}
...
Write now. Pure spoken word. No markdown. No headers.$BODY$,
  1,
  true,
  'Script Generator v1.0 §4 — Pass 1 Foundation prompt template'
)
ON CONFLICT (prompt_type) DO UPDATE
  SET prompt_text = EXCLUDED.prompt_text,
      version     = system_prompts.version + 1,
...
VALUES (
  'script_pass2',
  $BODY$Write PASS 2 of 3 for a YouTube masterclass titled: {subtopic}
...
Write now. Pure spoken word. No markdown. No headers.$BODY$,
  1,
  true,
  'Script Generator v1.0 §5 — Pass 2 Depth prompt template'
)
ON CONFLICT (prompt_type) DO UPDATE
  SET prompt_text = EXCLUDED.prompt_text,
      version     = system_prompts.version + 1,
...
VALUES (
  'script_pass3',
  $BODY$Write PASS 3 of 3 for a YouTube masterclass titled: {subtopic}
...
Write now. Pure spoken word. No markdown. No headers.$BODY$,
  1,
  true,
  'Script Generator v1.0 §6 — Pass 3 Resolution prompt template'
)
ON CONFLICT (prompt_type) DO UPDATE
  SET prompt_text = EXCLUDED.prompt_text,
      version     = system_prompts.version + 1,
...
VALUES (
  'script_evaluator',
  $BODY$You are a Script Quality Evaluator for a YouTube masterclass content pipeline. Your job is to score a script pass against defined metrics and return a structured verdict.
...
RESPOND IN THIS EXACT JSON FORMAT:
{
  ...
  1,
  true,
  'Script Generator v1.0 §7 — Quality Evaluator prompt template'
)
ON CONFLICT (prompt_type) DO UPDATE
  SET prompt_text = EXCLUDED.prompt_text,
      version     = system_prompts.version + 1,
...
VALUES (
  ...
  $BODY$RETRY ATTEMPT {ATTEMPT_NUMBER} of 3.
...
---$BODY$,
  1,
  true,
  'Script Generator v1.0 §8.2 — Retry injection template'
)
ON CONFLICT (prompt_type) DO UPDATE
  SET prompt_text = EXCLUDED.prompt_text,
      version     = system_prompts.version + 1,
...
VALUES (
  ...
  $BODY$Read the following script pass and extract:
...
RESPOND IN THIS JSON FORMAT:
{
  ...
  1,
  true,
  'Script Generator v1.0 §9.3 — Summary extraction prompt for Pass 3 context'
)
ON CONFLICT (prompt_type) DO UPDATE
  SET prompt_text = EXCLUDED.prompt_text,
      version     = system_prompts.version + 1,
