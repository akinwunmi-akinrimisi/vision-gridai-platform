...
### Stage C+: Scene Render Classification
...
#### AI Classification Prompt (Claude Haiku via Anthropic API)
...
After classification completes, the dashboard shows a new view within the production flow:
...
// - styleDna: project's style_dna string (used for font/vibe match)
```
...
**SceneClassificationReview** (`components/production/SceneClassificationReview.jsx`):
- Embedded within production flow between Script Approval (Gate 2) and Image Generation
- Summary bar: total scenes, Fal.ai count, Remotion count, estimated cost, estimated savings
- Scene list: filterable by render_method, each row shows scene number, render method badge (blue for Fal.ai, purple for Remotion), narration snippet, classification reasoning, template name (Remotion only)
...
- Hook: `useSceneClassification.js` — queries scenes, subscribes to Realtime for classification progress, provides mutation functions for overrides

**ProductionMonitor** — modified: new pipeline step between "Script Approved" and "Generating Images":
- Step label: "Scene Classification"
- Shows: green (reviewed), yellow (classified, awaiting review), blue-spinning (classifying), gray (pending)
- Displays Fal.ai/Remotion split counts inline
...
| C+ | Scene render classification → operator review | ~$0.03 |
| D1 | TTS audio (172 scenes) | ~$0.30 |
| D2 | Images (Fal.ai ~108 + Remotion ~64 scenes) | ~$3.24 |
...
*Previously ~$28/video with I2V/T2V. With Remotion hybrid (finance niche): ~$6.17/video. Remotion % varies by niche: finance/tech 30-40%, historical/lifestyle 10-15%.*
...
> Fal.ai for images. FFmpeg Ken Burns for motion. Remotion for kinetic captions. Agency Agents for specialist expertise.
